...
def build_gru(window, n_features, horizon):
    model = Sequential([
        GRU(96, return_sequences=True, input_shape=(window, n_features)),
        BatchNormalization(),
        Dropout(0.15),
...
def plot_results(y_true, y_pred, station, n=500):
    ...
    axes[0].plot(y_pred[:n], label="GRU Predicted")
...
def pick_positive_predictions(ts, Xte, yte, model_pred_sc):
    ...
    baseline_sc = np.repeat(Xte[:, -1, -1].reshape(-1, 1), HORIZON, axis=1)
...
for station in STATIONS:
    ...
    model = build_gru(WINDOW_SIZE, N_FEATURES, HORIZON)
    ckpt = MODEL_DIR / f"gru_aqi_{station}.keras"
    ...
    joblib.dump(fs, MODEL_DIR / f"gru_feat_scaler_{station}.pkl")
    joblib.dump(ts, MODEL_DIR / f"gru_tgt_scaler_{station}.pkl")
    plot_results(ytrue1, ypred1, station)

    all_metrics[station] = m
    print(f"{station} GRU test R2: {m['R2']}")

with open(RESULT_DIR / "gru_metrics.json", "w") as f:
